...
module.exports = function(rewrites) {
  rewrites = (('object' == typeof rewrites) && (Array == rewrites.constructor)) ? rewrites : [ rewrites ];
  rewrites = rewrites.map(function(item) {
    if ('object' != typeof item) return undefined;
    if (!item.match || !item.target) return undefined;
    var match = (('function' == typeof item.match) && (item.match.constructor == RegExp)) ? item.match : new RegExp(String(item.match),'i');
  ...
  return function(request, response, callback) {
    var actions = rewrites.map(function(item) { return item; });
    actions.unshift(function(callback) {
      callback(undefined, request.url);
    });
    async.waterfall(actions, function(err, url) {
      if (err) return callback(err, 0);
      if (!url.length) return callback(new Error('URL Rewrites to Nothing'), 0);
      request.url = url;
